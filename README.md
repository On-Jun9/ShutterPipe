...
`skip`, `rename`, `quarantine`은 계획 이후 생긴 파일도 덮어쓰지 않도록 목적지 파일시스템의 원자적 exclusive rename 또는 hard link 기능을 사용합니다. 두 기능을 모두 지원하지 않는 파일시스템에서는 부분 파일을 최종본으로 노출하지 않고 해당 작업을 안전하게 실패 처리합니다.
